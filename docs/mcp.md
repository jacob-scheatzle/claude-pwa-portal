...
| `create_app(files, icon_base64=None, replace=false)` | **Preferred over MCP.** Install/update an app from its source **files as text** — `files` maps filename → content and must include `portal.json` + the entry HTML. The portal assembles the bundle server-side, so there's no zip and no base64 to corrupt. Icon is optional: pass `icon_base64`, include an `.svg` in `files`, or one is generated (192×192 accent square + the app's initial, per the skill's icon rules). `replace=true` preserves per-user storage. |
| `upload_app(filename, zip_base64, replace=false)` | Install a **pre-packaged** `.zip` (base64-encoded). Use `create_app` instead when an assistant is authoring the app; reserve this for a real zip you can base64 from a file (`base64 -w0 app.zip`). Large bundles can be truncated over MCP — prefer the web UI. `replace=true` updates in place, **preserving per-user storage**. |
